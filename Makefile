...
PROG=$(BIN_DIR)/rdma-bench $(BIN_DIR)/rc_connection $(BIN_DIR)/test_bitmap $(BIN_DIR)/sg_list $(BIN_DIR)/sg_list_perf $(BIN_DIR)/ping_pong $(BIN_DIR)/test_socket $(BIN_DIR)/ping_pong_cmplt_cnl
...
$(BIN_DIR)/ping_pong_cmplt_cnl: $(SRC_OBJS)
	@mkdir -p $(BIN_DIR)
	@$(CC) $(CFLAGS) $(INCLUDES) -o $@ $(EXAMPLE_DIR)/ping_pong_cmplt_cnl.o $(SRC_OBJS) $(LDFLAGS) $(LDLIBS)
